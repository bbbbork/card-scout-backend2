...
app.get('/api/tcg/search', async (req, res) => {
  ...
  const lang = req.query.lang || 'en'; // 'en' or 'jp'
  if (!q) return res.json({ data: [], totalCount: 0 });
  try {
    // Build smart search query - detect card numbers, set names, etc.
    let searchQuery;
    const hasSlash = q.includes('/');
    const hasHash = q.includes('#');
    const numMatch = q.match(/(\d+)\/(\d+)/); // e.g. "199/165"
    
    if (numMatch) {
      // User is searching by card number like "199/165" or "charizard 199/165"
      const textPart = q.replace(/\d+\/\d+/, '').trim();
      searchQuery = `number:"${numMatch[1]}"`;
      if (textPart) searchQuery += ` name:"*${textPart}*"`;
    } else if (hasHash) {
      // User searching like "charizard #4"  
      const parts = q.split('#');
      const name = parts[0].trim();
      const num = parts[1].trim();
      searchQuery = name ? `name:"*${name}*" number:"${num}"` : `number:"${num}"`;
    } else {
      searchQuery = `name:"*${q}*"`;
    }
    ...
    // If Japanese requested, also try PokemonPriceTracker for JP cards
    let pptResults = [];
    if (lang === 'jp' && PPT_API_KEY) {
      try {
        const pptRes = await fetch(`https://www.pokemonpricetracker.com/api/v2/cards?search=${encodeURIComponent(q)}&language=japanese&limit=20`, {
          headers: { 'Authorization': `Bearer ${PPT_API_KEY}` },
        });
        if (pptRes.ok) {
          const pptJson = await pptRes.json();
          pptResults = (pptJson.data || []).map(c => ({
            tcgId: c.tcgPlayerId || c.id || null,
            name: c.name || q,
            set: c.setName || '',
            setId: c.setId || '',
            number: c.number || '',
            ...
            year: c.year || null,
            imageSmall: c.imageUrl || null,
            imageLarge: c.imageUrlLarge || c.imageUrl || null,
            tcgPlayerPrice: c.prices?.market || c.prices?.mid || null,
            cardmarketPrice: null,
            source: 'ppt-jp',
          }));
        }
      } catch (e) { console.error('[PPT JP Search]', e.message); }
    }

    const allCards = lang === 'jp' ? [...pptResults, ...cards] : cards;
    res.json({ data: allCards, totalCount: json.totalCount || allCards.length, page, pageSize, lang });
  } catch (e) {
...
const CARDS = [
  ...
  {id:85, name:'Ralts',                   set:'Scarlet & Violet',     year:2023, number:'211/198', rarity:'Illustration Rare',       rawPrice:31,     psa10:150,     tcgId:'sv1-211',    tags:['ir','sleeper'],  era:'hyper'},
  {id:86, name:'Drowzee',                 set:'Scarlet & Violet',     year:2023, number:'210/198', rarity:'Illustration Rare',       rawPrice:41,     psa10:180,     tcgId:'sv1-210',    tags:['ir','sleeper'],  era:'hyper'},
  ...
  {id:106,name:'Eevee',                   set:'Twilight Masquerade',  year:2024, number:'188/167',      rarity:'Illustration Rare',       rawPrice:18,     psa10:85,      tcgId:'sv6-188',       tags:['ir','eeveelution'],           era:'hyper'},
  {id:107,name:'Infernape',            set:'Twilight Masquerade',  year:2024, number:'173/167',     rarity:'Special Illustration Rare',rawPrice:40,     psa10:175,     tcgId:'sv6-173',       tags:['sir','starter'],              era:'hyper'},
  {id:108,name:'Growlithe',               set:'Twilight Masquerade',  year:2024, number:'181/167',      rarity:'Illustration Rare',       rawPrice:15,     psa10:70,      tcgId:'sv6-181',       tags:['ir','popular'],               era:'hyper'},
  {id:109,name:'Terapagos ex',            set:'Stellar Crown',        year:2024, number:'175/142', rarity:'Special Illustration Rare',rawPrice:45,     psa10:220,     tcgId:'sv7-175',       tags:['sir','legendary'],            era:'hyper'},
  {id:110,name:'Arceus',                  set:'Shrouded Fable',       year:2024, number:'80/64',   rarity:'Illustration Rare',       rawPrice:24,     psa10:110,     tcgId:'sv6pt5-80',       tags:['ir','legendary'],             era:'hyper'},
  {id:111,name:'Pikachu ex',              set:'Surging Sparks',       year:2024, number:'238/191', rarity:'Special Illustration Rare',rawPrice:200,    psa10:600,     tcgId:'sv8-238',       tags:['sir','pikachu'],              era:'hyper'},
  {id:112,name:'Hydreigon ex',            set:'Surging Sparks',       year:2024, number:'240/191', rarity:'Special Illustration Rare',rawPrice:140,    psa10:400,     tcgId:'sv8-240',       tags:['sir','dragon'],               era:'hyper'},
  {id:113,name:'Latias ex',               set:'Surging Sparks',       year:2024, number:'239/191', rarity:'Special Illustration Rare',rawPrice:140,    psa10:420,     tcgId:'sv8-239',       tags:['sir','legendary'],            era:'hyper'},
  {id:114,name:"Lisia's Appeal",          set:'Surging Sparks',       year:2024, number:'242/191', rarity:'Special Illustration Rare',rawPrice:55,     psa10:240,     tcgId:'sv8-242',       tags:['sir','trainer'],              era:'hyper'},
  {id:115,name:'Milotic ex',               set:'Surging Sparks',       year:2024, number:'237/191', rarity:'Special Illustration Rare',rawPrice:90,     psa10:400,     tcgId:'sv8-237',    tags:['sir','popular'],           era:'hyper'},
  {id:116,name:'Umbreon ex',              set:'Prismatic Evolutions', year:2025, number:'161/131', rarity:'Special Illustration Rare',rawPrice:882,    psa10:4652,    tcgId:'sv8pt5-161',       tags:['sir','eeveelution','grail'],  era:'hyper'},
  {id:117,name:'Sylveon ex',              set:'Prismatic Evolutions', year:2025, number:'162/131', rarity:'Special Illustration Rare',rawPrice:300,    psa10:1100,    tcgId:'sv8pt5-162',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:118,name:'Leafeon ex',              set:'Prismatic Evolutions', year:2025, number:'155/131', rarity:'Special Illustration Rare',rawPrice:220,    psa10:800,     tcgId:'sv8pt5-155',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:119,name:'Glaceon ex',              set:'Prismatic Evolutions', year:2025, number:'157/131', rarity:'Special Illustration Rare',rawPrice:125,    psa10:480,     tcgId:'sv8pt5-157',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:120,name:'Espeon ex',               set:'Prismatic Evolutions', year:2025, number:'156/131', rarity:'Special Illustration Rare',rawPrice:135,    psa10:540,     tcgId:'sv8pt5-156',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:121,name:'Flareon ex',              set:'Prismatic Evolutions', year:2025, number:'153/131', rarity:'Special Illustration Rare',rawPrice:85,     psa10:360,     tcgId:'sv8pt5-153',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:122,name:'Jolteon ex',              set:'Prismatic Evolutions', year:2025, number:'154/131', rarity:'Special Illustration Rare',rawPrice:90,     psa10:380,     tcgId:'sv8pt5-154',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:123,name:'Vaporeon ex',             set:'Prismatic Evolutions', year:2025, number:'158/131', rarity:'Special Illustration Rare',rawPrice:80,     psa10:340,     tcgId:'sv8pt5-158',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:124,name:'Eevee',                   set:'Prismatic Evolutions', year:2025, number:'159/131', rarity:'Special Illustration Rare',rawPrice:65,     psa10:260,     tcgId:'sv8pt5-159',       tags:['sir','eeveelution'],          era:'hyper'},
  {id:125,name:'Umbreon ex',              set:'Prismatic Evolutions', year:2025, number:'060/131', rarity:'Double Rare',             rawPrice:15,     psa10:60,      tcgId:'sv8pt5-60',       tags:['eeveelution','affordable'],   era:'hyper'},
  ...
  {id:139,name:'Alolan Exeggutor ex',     set:'Surging Sparks',       year:2024, number:'236/191', rarity:'Special Illustration Rare',rawPrice:24,     psa10:110,     tcgId:'sv8-236',       tags:['sir','meme'],                 era:'hyper'},
