...
// ═══════════════════════════════════════════════════════════════════════════
// PRICE ENGINE — PokemonPriceTracker (primary) + eBay Browse API (backup)
// ═══════════════════════════════════════════════════════════════════════════
...
// ── PokemonPriceTracker (primary) ────────────────────────────────────────
// Uses tcgPlayerId (e.g. 'base1-4') which matches our tcgId field
// Free tier: 100 credits/day — plenty for 140 cards
async function fetchPPTPrice(tcgId) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[PPT] Failed for ${tcgId}: ${res.status}`);
      return null;
    }
    ...
    const p = card.prices;
    return {
      median: p.market || p.mid || null,
      avg: p.mid || p.market || null,
      low: p.low || null,
      high: p.high || null,
      count: null,
      source: 'ppt',
    ...
    console.error(`[PPT] Error for ${tcgId}:`, e.message);
    return null;
  }
}

// ── eBay Browse API (backup for cards not in PPT) ────────────────────────
...
// ── Unified price fetcher: PPT first, eBay fallback ──────────────────────
async function fetchBestPrice(card) {
  // Try PokemonPriceTracker first (no rate limit issues, uses tcgId)
  if (card.tcgId && card.tcgId !== 'none') {
    const ppt = await fetchPPTPrice(card.tcgId);
    if (ppt && ppt.median) {
      console.log(`[Price] ✓ ${card.name} (${card.set}) = $${ppt.median} via PPT`);
      return ppt;
    }
  }
  // Fall back to eBay for cards not in PPT (newer sets, promos)
  const ebay = await fetchEbayPrice(card.name, card.set, card.number);
  if (ebay) {
    console.log(`[Price] ✓ ${card.name} (${card.set}) = $${ebay.median} via eBay`);
    return ebay;
  }
  console.log(`[Price] ✗ ${card.name} (${card.set}) — no data from any source`);
  return null;
}
...
// ── Refresh: PPT cards go fast (no rate limit), eBay cards go slow ───────
async function refreshAllPrices(forceAll = false) {
  ...
  console.log(`[Refresh] Starting: ${toRefresh.length} cards to fetch (${CARDS.length - toRefresh.length} already cached)`);
  if (toRefresh.length === 0) { console.log('[Refresh] All cards cached!'); return { ok: 0, fail: 0, skipped: CARDS.length }; }

  // Split into PPT-eligible (have tcgId) and eBay-only (tcgId='none')
  const pptCards = toRefresh.filter(c => c.tcgId && c.tcgId !== 'none');
  const ebayCards = toRefresh.filter(c => !c.tcgId || c.tcgId === 'none');

  let ok = 0, fail = 0;

  // Phase 1: PPT cards — fast, 500ms delay (no strict rate limit)
  if (pptCards.length > 0) {
    console.log(`[Refresh] Phase 1: ${pptCards.length} cards via PokemonPriceTracker`);
    for (const c of pptCards) {
      try {
        ...
        if (price && price.median) {
          const key = `${c.name}|${c.set}|${c.number}`.toLowerCase();
          const now = new Date().toISOString();
          db.prepare('INSERT OR REPLACE INTO ebay_price_cache (card_key,card_id,card_name,set_name,card_number,median_price,avg_price,low_price,high_price,result_count,last_updated) VALUES (?,?,?,?,?,?,?,?,?,?,?)').run(key, c.id, c.name, c.set, c.number, price.median, price.avg || price.median, price.low || price.median, price.high || price.median, price.count || 0, now);
          ok++;
        } else { fail++; }
      } catch (e) { fail++; }
      await new Promise(r => setTimeout(r, 600)); // 100 credits/day = safe at 600ms
    }
    console.log(`[Refresh] Phase 1 done: ${ok} ok, ${fail} failed`);
  }

  // Phase 2: eBay-only cards — slow, batched (rate limited)
  if (ebayCards.length > 0) {
    console.log(`[Refresh] Phase 2: ${ebayCards.length} cards via eBay (slower)`);
    const BATCH = 15;
    for (let i = 0; i < ebayCards.length; i++) {
      ...
      try {
        ...
        if (price) {
          const key = `${c.name}|${c.set}|${c.number}`.toLowerCase();
          const now = new Date().toISOString();
          db.prepare('INSERT OR REPLACE INTO ebay_price_cache (card_key,card_id,card_name,set_name,card_number,median_price,avg_price,low_price,high_price,result_count,last_updated) VALUES (?,?,?,?,?,?,?,?,?,?,?)').run(key, c.id, c.name, c.set, c.number, price.median, price.avg, price.low, price.high, price.count, now);
          ok++;
        } else { fail++; }
      } catch (e) { fail++; }
      await new Promise(r => setTimeout(r, 5000));
      if ((i + 1) % BATCH === 0 && i + 1 < ebayCards.length) {
        console.log(`[Refresh] eBay batch pause — waiting 90s...`);
        await new Promise(r => setTimeout(r, 90000));
      }
    }
  }

  console.log(`[Refresh] Complete: ${ok} ok, ${fail} failed, ${CARDS.length - toRefresh.length} skipped (cached)`);
...
// Health
app.get('/api/health', (req, res) => res.json({ status: 'ok', cards: CARDS.length, ppt: !!PPT_API_KEY, ebay: !!(EBAY_CLIENT_ID && EBAY_CLIENT_SECRET), uptime: process.uptime() }));
...
app.get('/api/ebay/status', (req, res) => {
  ...
  res.json({
    pptEnabled: !!PPT_API_KEY,
    ebayEnabled: !!(EBAY_CLIENT_ID && EBAY_CLIENT_SECRET),
    cardBreakdown: { pptEligible: pptCount, ebayOnly: ebayCount, total: CARDS.length },
...
app.listen(PORT, () => {
  ...
  console.log(`[Server] ${CARDS.length} cards loaded (${pptCount} via PPT, ${ebayCount} via eBay)`);
  console.log(`[Server] PPT: ${PPT_API_KEY ? 'key found' : 'no key — set PPT_API_KEY'}`);
  console.log(`[Server] eBay: ${EBAY_CLIENT_ID ? 'credentials found' : 'no credentials'}`);

  // Auto-refresh every 12 hours if any price source is configured
